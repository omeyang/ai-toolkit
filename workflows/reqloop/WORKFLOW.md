...
**安全例外（硬性覆盖默认规则）**：
冲突条目命中安全关键词（鉴权 / 审计 / 限流 / PII / 加密 / 幂等 / 注入等）时，
**不走"代码为准"**，改为以 PRD 为准并阻塞验收，自动调起 `security-auditor`。
完整关键词列表与处置规则见 `stages/03-backspec.md`。

### 2b. 判定三元组（verdict + confidence + evidence）

阶段 3、4、5、6 的每一条 AI 判定都必须输出三元组：
...
- [04.5-impact-radius.md](stages/04.5-impact-radius.md) — 负向影响分析
